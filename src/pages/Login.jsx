    ...
    return (
        <div  className="w-screen h-screen grid grid-cols-5">
            ...
            <div className="col-span-3 bg-slate-100 ps-5 sm:px-36 flex flex-col items-center justify-center">
                ...
                <form className=" flex flex-col gap-5 w-11/12  lg:w-1/2 me-auto" onSubmit={handleLogin}>
                    <label className="flex flex-col text-start ">
                        Email
                        <input  required className="border sm:text-base text-sm outline-none rounded-2xl pe-1 ps-2 sm:px-4 py-1" type="text" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" />
                    </label>
                    <label className="flex flex-col text-start">
                        Password
